...
test.describe('Responsive Design', () => {
  test.describe('Desktop Layout', () => {
    ...
    test('desktop navigation is visible', async ({ page }) => {
      ...
      await expect(page.locator('header nav.hidden.md\\:flex a[href="/faq"]')).toBeVisible();
      await expect(page.locator('header nav.hidden.md\\:flex a[href="/process"]')).toBeVisible();
      await expect(page.locator('header nav.hidden.md\\:flex a[href="/about"]')).toBeVisible();
    });
  ...
  test.describe('Mobile Layout', () => {
    ...
    test('mobile menu opens and shows links', async ({ page }) => {
      ...
      await expect(page.locator('header nav.md\\:hidden a[href="/faq"]')).toBeVisible();
    });
  ...
  test('portfolio images have lazy loading', async ({ page }) => {
    ...
    if (count > 0) {
      for (let i = 0; i < Math.min(count, 3); i++) {
        const loadingAttr = await portfolioImages.nth(i).getAttribute('loading');
