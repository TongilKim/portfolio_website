...
test.describe('Contact Form', () => {
  ...
  test('validates email format', async ({ page }) => {
    ...
    const hasEmailError = await page.locator('.text-red-600, .text-red-500').count() > 0;
  ...
  test('submit button is enabled after filling form', async ({ page }) => {
    ...
    if (await projectField.count() > 0) {
      ...
      await projectField.first().pressSequentially('Website Project', { delay: 10 });
    }

    const messageField = form.locator('textarea').first();
    await messageField.click();
    await messageField.pressSequentially('This is a test message for the form.', { delay: 5 });
  ...
  test('form works in Korean language', async ({ page }) => {
    ...
    await form.locator('input[name="name"], input[placeholder*="name" i]').first().fill('테스트 사용자');
    await form.locator('input[type="email"]').first().fill('test@example.com');
    await form.locator('textarea').first().fill('테스트 메시지입니다. 연락 양식을 테스트합니다.');

    // Verify fields are filled
    await expect(form.locator('input[name="name"], input[placeholder*="name" i]').first()).toHaveValue('테스트 사용자');
  });
});
